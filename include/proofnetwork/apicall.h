...
struct ApiCallSuccessCallbackGenerator
{
    ...
    static std::function<bool(qulonglong, Result)> result(qulonglong &currentOperationId, Result &result, Signal)
    {
...
struct ApiCall
{
    ...
    {
        return tasks::run(
                   [callee, method, signal, args...](void) -> Result {
                       if (callee->isLoggedOut())
                           return WithFailure(QStringLiteral("API is not logged in"), NETWORK_MODULE_CODE,
                                              NetworkErrorCode::AuthCredentialsError);
                       qulonglong currentOperationId = 0;
                       Result result;
                       auto callback = detail::ApiCallSuccessCallbackGenerator<Result>::result(currentOperationId,
                                                                                               result, signal);
                       RestApiError error;
                       tasks::addSignalWaiter(callee, signal, callback);
                       tasks::addSignalWaiter(callee, &Proof::AbstractRestApi::apiErrorOccurred,
                                              AbstractRestApi::generateErrorCallback(currentOperationId, error));
                       currentOperationId = (*callee.*method)(args...);
                       if (!currentOperationId)
                           return WithFailure(QStringLiteral("Invalid arguments"), NETWORK_MODULE_CODE,
                   ...
                   tasks::RestrictionType::Custom, callee->restClient()->host())
            ->recoverWith([attempts, callee, method, signal, args...](const Failure &failure) -> FutureSP<Result> {
                if (attempts <= 1)
                    return WithFailure(failure);
                else
                    return exec(attempts - 1, callee, method, signal, args...);
...
struct ApiCall<void>
{
    ...
    {
        return tasks::run(
                   [callee, method, signal, args...](void) -> bool {
                       if (callee->isLoggedOut())
                           return WithFailure(QStringLiteral("API is not logged in"), NETWORK_MODULE_CODE,
                                              NetworkErrorCode::AuthCredentialsError);
                       qulonglong currentOperationId = 0;
