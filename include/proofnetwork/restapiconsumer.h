...
public:
    ...
    {
        return Proof::AbstractRestApi::chainedApiCall(taskChain, callee, method, signal, std::forward<Result>(result), args...);
    }
    template <class Callee, class Result, class Method, class Signal, class ...Args>
    ...
    {
        return Proof::AbstractRestApi::chainedApiCall(attempts, taskChain, callee, method, signal, std::forward<Result>(result), args...);
    }
    template <class Callee, class Method, class Signal, class ...Args>
    RestApiError chainedApiCallWithoutResult(const TaskChainSP &taskChain, Callee *callee, Method method,
                                             Signal signal, Args... args) const
    {
        return Proof::AbstractRestApi::chainedApiCallWithoutResult(taskChain, callee, method, signal, args...);
    }
    template <class Callee, class Method, class Signal, class ...Args>
    RestApiError chainedApiCallWithoutResult(int attempts, const TaskChainSP &taskChain, Callee *callee,
